...
export class ServiceRequestRepository {
  ...
  find(id: string) { return this.prisma.serviceRequest.findUnique({ where: { id }, include: { service: true, customer: { include: { customerProfile: { include: { addresses: true } } } }, assignedVendor: true, payments: true } }); }
  ...
  technicians(branchId?: string) { return this.prisma.vendor.findMany({ where:{ role:"TECHNICIAN", verificationStatus:"VERIFIED", profileStatus:"PUBLISHED", deletedAt:null, branchId: branchId || undefined }, select:{id:true,fullName:true,phone:true,latitude:true,longitude:true} }); }
}
