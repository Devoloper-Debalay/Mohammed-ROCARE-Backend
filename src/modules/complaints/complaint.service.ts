...
@injectable()export class ComplaintService{
 ...
 async reply(id:string,adminId:string,d:any){const c=await prisma.complaint.findUnique({where:{id}});if(!c)throw createHttpError(404,"Complaint not found.");return prisma.complaint.update({where:{id},data:{resolvedBy:adminId,status:d.status||"IN_REVIEW",adminReply:d.message}});}
}
